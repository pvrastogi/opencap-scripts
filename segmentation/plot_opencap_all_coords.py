...
FILE SELECTION
    For sessions where both <trial>.mot and <trial>_shoulder.mot exist, the
    _shoulder version is preferred automatically -- that is the harmonized
    40-column file, and it is the same file the segmentation scripts read
    (see load_mot in seg_common.py). For sessions processed natively on the
    shoulder model the plain <trial>.mot is already correct and is used
    as-is. Any file with "deleted" in the name is skipped.

USAGE
    Two ways, both supported:

    1. QC SOP. Drop this file into a session's OpenSimData/Kinematics/
       folder, next to plot_opencap_balance.py, and run it with no
       arguments. It finds its own session by walking up from its location:
           python3 OpenSimData/Kinematics/plot_opencap_all_coords.py

    2. Segmentation SOP. Pass a session folder or label. run_all.py calls it
       this way as its "coord plots" step:
           python plot_opencap_all_coords.py "/path/to/patient folder"
