...
Mirrors the structure of plot_opencap_ue.py, but for the gait tasks:
gait_1, gait-start, gait-headturn, gait-tandem, gait-pivot, march, grapevine.
...
- pelvis_tilt / pelvis_list / pelvis_rotation are flagged by the OpenCap
  ...
  coordinates in meters) together on one row.
- march and grapevine are also plotted in plot_opencap_balance.py with a
  different angle set (they're relevant to both categories).

USAGE:
    python3 plot_opencap_gait.py

Output:
    PNG files saved to ./output_plots/gait/
"""
...
OUTPUT_DIR = os.path.join(ROOT_DIR, "Outputs", "AnglePlots", "gait")
...
TASKS = {
    ...
    "gait_headturn": {
        "file": "gait-headturn.mot",
        # Same angle set as gait_1/gait_start on purpose: the point of this
        # task is to see how the added cognitive/motor load of turning the
        # head while walking affects gait itself, so it should be directly
        # comparable to the baseline gait tasks.
        "angles": ["hip_flexion", "knee_angle", "ankle_angle", "pelvis_position"],
    },
    "gait_tandem": {
        "file": "gait-tandem.mot",
        "angles": ["hip_flexion", "knee_angle", "ankle_angle", "hip_adduction"],
    },
    "gait_pivot": {
        "file": "gait-pivot.mot",
        "angles": ["hip_flexion", "knee_angle", "ankle_angle", "hip_rotation"],
    },
    "march": {
        "file": "march.mot",
        "angles": ["hip_flexion", "knee_angle", "ankle_angle"],
    },
    "grapevine": {
        "file": "grapevine.mot",
        "angles": ["hip_adduction", "hip_rotation", "knee_angle", "pelvis_position"],
    },
}

# Readable labels for titles/axis labels
ANGLE_LABELS = {
...
def plot_task(task_name, task_config):
    ...
    fig.suptitle(f"OpenCap Gait Task: {task_name}  ({task_config['file']})", fontsize=14, fontweight="bold")
